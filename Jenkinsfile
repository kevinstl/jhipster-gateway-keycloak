pipeline {
  ...
  environment {
    ORG = 'kevinstl'
      APP_NAME = 'jhipster-gateway-keycloak'
...
//                sh "git config remote.origin.url https://github.com/kevinstl/jhipster-gateway-keycloak.git"
    ...
    stage('Build Release - Tag') {
      ...
      steps {
        container('jx-base') {
            ...
            sh 'cat ./charts/jhipster-gateway-keycloak/Chart.yaml'
            sh 'cat ./charts/jhipster-gateway-keycloak/values.yaml'
        ...
        script {
          if (kubeEnv?.trim() == 'local') {
              container('jx-base') {
                  ...
                  sh "ls -al"
                  sh "mkdir -p /root/.m2/jhipster-gateway-keycloak/node"
                  sh "ln -s /root/.m2/jhipster-gateway-keycloak/node"
                  sh "ls -al /root/.m2/jhipster-gateway-keycloak/node"

                  sh "mkdir -p /root/.m2/jhipster-gateway-keycloak/node_modules"
                  sh "ln -s /root/.m2/jhipster-gateway-keycloak/node_modules"
                  sh "ls -al /root/.m2/jhipster-gateway-keycloak/node_modules"

//                  sh "mkdir -p /root/.m2/jhipster-gateway-keycloak/target"
//                  sh "chmod -R 777 /root/.m2/jhipster-gateway-keycloak/target"
//                  sh "ln -s /root/.m2/jhipster-gateway-keycloak/target"
//                  sh "ls -al /root/.m2/jhipster-gateway-keycloak/target"
...
//        container('jx-base') {
//
//          dir('charts/jhipster-gateway-keycloak') {
    ...
    script {
        ...
        dir(envProjectDir) {
            container('jx-base') {
                ...
                sh " ./scripts/replace-version.sh . ./requirements.yaml jhipster-gateway-keycloak \$(cat ../../VERSION)"
...
//                sh 'jx step helm apply --wait=false'
                sh 'jx step helm apply -n jhipster-gateway-keycloak --wait=false'
            }
        }
    }
}
